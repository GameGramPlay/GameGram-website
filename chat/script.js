...
let peer;
let bootstrapId = null; // current bootstrap peer ID
...
loginBtn.onclick = async () => {
    ...
    if (!nick || !room) { alert("Enter nickname & room!"); return; }
    ...
    try { await startPeerWithFallbacks(); } 
    catch (err) { addSystem("Failed to join: " + err); login.style.display = 'block'; }
};
...
function tryPeerHostsSequentially(hosts, idx) {
    if(idx >= hosts.length){ addSystem("All public PeerJS endpoints failed."); return; }
    ...
    startPeerWithOptions({
        ...
        if(err){ cleanupPeer(); setTimeout(()=>tryPeerHostsSequentially(hosts, idx+1),500); }
    });

    setTimeout(()=>{
        if(!peer?.id){ cleanupPeer(); tryPeerHostsSequentially(hosts, idx+1); }
    }, PEER_OPEN_TIMEOUT);
}
...
function discoverPeers(){
    ...
    known.forEach(id=>enqueuePeer(id));
}

// --- Connection queue functions ---
function enqueuePeer(peerId){ if(!peers.has(peerId) && peerId !== peer.id && !connectQueue.includes(peerId)){ connectQueue.push(peerId); processQueue(); } }
...
function setupConn(conn){
    conn.on('open', ()=>{
        ...
        conn.send({ type:'join', id:peer.id, nickname, color:nickColor, status, bootstrapId });
        conn.send({ type:'history', history: chatHistory });
        conn.send({ type:'peerlist', peers:Array.from(peers.keys()).concat([peer.id]) });
    });
    ...
    conn.on('data', data=>{
        switch(data.type){
            ...
            case 'bootstrap-update': bootstrapId=data.bootstrapId; addSystem(`Bootstrap updated: ${bootstrapId}`); break;
            case 'request-peerlist': conn.send({ type:'peerlist', peers:Array.from(peers.keys()).concat([peer.id]) }); break;
        }
...
function handlePeerDisconnect(pid){
    peers.delete(pid);
    if(users[pid]) users[pid].status="offline";
    updateUserList();
    enqueuePeer(pid);
    if(pid===bootstrapId) { electBootstrap(); reconnectViaBootstrap(); }
}

// ---------- Bootstrap / Reconnect ----------
function electBootstrap(){
    if(peers.size===0) bootstrapId=null;
    else bootstrapId=Array.from(peers.keys())[0];
}

function reconnectViaBootstrap(retries=0){
    if(retries>5){ addSystem("Max bootstrap reconnection attempts reached"); return; }
    if(!bootstrapId || bootstrapId===peer.id) return;

    addSystem(`Reconnecting via bootstrap ${bootstrapId} attempt ${retries+1}`);
    const conn = peer.connect(bootstrapId,{reliable:true});
    let success=false;

    conn.on('open', ()=>{
        addSystem(`Connected to bootstrap ${bootstrapId}`);
        setupConn(conn);
        conn.send({ type:'request-peerlist' });
        success=true;
    });
    conn.on('error', ()=>{ if(!success) setTimeout(()=>reconnectViaBootstrap(retries+1),3000+retries*2000); });
    conn.on('close', ()=>{ if(!success) setTimeout(()=>reconnectViaBootstrap(retries+1),3000+retries*2000); });
}
...
function handleIncomingMsg(msg){ if(!chatHistory.find(m=>m.id===msg.id)){ addMsg(msg.nickname,msg.text,msg.time,msg.color,msg.id); safeBroadcast(msg); } }

function safeBroadcast(data){ if(peers.size>0){ broadcast(data); } else { setTimeout(()=>safeBroadcast(data),15000); } }
...
function broadcastJoin(conn){
    const data={ type:'join', id:peer.id, nickname, color:nickColor, status, bootstrapId };
...
setInterval(()=>{ if(peer && peer.open && bootstrapId && bootstrapId!==peer.id && !peers.has(bootstrapId)){ reconnectViaBootstrap(); } },10000+Math.random()*5000);
