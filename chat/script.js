...
async function makePeerId() {
  // Try localStorage or first available sequential ID
  const known = new Set(JSON.parse(localStorage.getItem("knownPeers")||"[]"));
  for (let i = 1; i <= 1000; i++) {
    const candidate = `gamegramuser${i}`;
    if (!known.has(candidate) && !knownPeers.has(candidate)) return candidate;
  }
  return `gamegramuser${Date.now()}`;
}
...
function tryPeerHostsSequentially(hosts, attemptIndex) {
  if (attemptIndex >= hosts.length) { addSystem("All public PeerJS endpoints failed."); return; }
  ...
  startPeerWithOptions({
    ...
    if (err) { cleanupPeer(); setTimeout(()=> tryPeerHostsSequentially(hosts, attemptIndex + 1), 500); }
  });
  setTimeout(() => { if (!peer?.id) { cleanupPeer(); tryPeerHostsSequentially(hosts, attemptIndex + 1); } }, PEER_OPEN_TIMEOUT);
}
...
async function startPeerWithOptions(opts, callback) {
  ...
  peer.on('open', id => {
    ...
    peer.on('connection', setupConnFromIncoming);
...
function discoverPeers() {
  for (let i = 1; i <= 100; i++) { enqueuePeer(`gamegramuser${i}`); }
}

// --- Connection queue functions ---
function enqueuePeer(peerId) {
  if (!peers.has(peerId) && peerId !== peer.id && !connectQueue.includes(peerId)) { connectQueue.push(peerId); processQueue(); }
}
...
function setupConnFromIncoming(conn) {
...
function broadcast(data,except){
  peers.forEach(c=>{ try{ if(c.open && c!==except) c.send(data); } catch(e){ console.warn("Broadcast failed",c.peer,e); } });
}
